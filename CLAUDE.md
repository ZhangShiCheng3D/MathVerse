...
- **`app/services/deeptutor_ws.py`** — WebSocket client for the verified endpoints, aggregating streamed events:
  - `chat(message, mode)` → `WS /api/v1/chat`; modes `chat`/`solve`/`quiz`/`research`/`visualize`. Events: `{type: session|status|stream(content)|sources|result(content)|error}`. **The stream is prose, not structured.**
...
### Frontend
- `src/services/api.ts` — `request()` wrapper over `Taro.request`, token storage + 401 refresh, surfaces 429. `BASE_URL` from `process.env.TARO_APP_API_URL` (default `https://api.shuxuejie.com`).
...
Work lives on branch **`improve/design-review-hardening`** → **PR #1** (`ZhangShiCheng3D/MathVerse`, open, ~10 commits). Tests went **19 → 65, all green**. Done in rounds:
...
4. **DeepTutor integration rewrite** — `deeptutor_ws.py` (verified WS contract) + circuit breaker; `deep_solve` structured-JSON parsing with prose fallback; in-process WS contract tests.
...
**Pending — needs a live DeepTutor instance (API keys + image):**
- Real-machine regression: confirm `chat(solve)` structured compliance, `vision/solve` + `question/judge` event fields against the running `/docs`.
- Migrate `generate_quiz` / lecture from legacy `_post` to `question/*` WS once event schema confirmed.
- Frontend streaming UI; route `exercise/grade` to `quiz_judge`; OCR via `vision_solve`.
