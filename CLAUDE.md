...
pytest                                          # all tests (138 currently, all green)
...
- `tutor` (`app/routes/tutor.py`): **`WS /ws/tutor`** — bidirectional proxy over DeepTutor's unified_ws **turn runtime** (the agentic core), incl. the `ask_user` interactive loop; tenancy-scoped session + quota + content-filter + archive. (This is the first MathVerse dependency on DeepTutor **face A** — see C3 deploy red-line.)
